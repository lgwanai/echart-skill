---
description: "通过统一 SQL runner 执行 DuckDB/PostgreSQL/MySQL 查询"
argument-hint: "<SQL>"
---

# /echart-query

映射到 echart-skill 原始指令 `/query`。

## 任务

根据 schema 与当前生效统计口径生成 SQL，并使用 scripts/sql_runner.py 执行；可以用 --sql 直接传 SQL，也可以用 --file 执行 SQL 文件。禁止临时 Python 连接数据库。不要把大表明细读入模型上下文，只返回必要聚合、样例或输出文件路径。
...
- `/echart-query SELECT * FROM sales LIMIT 10`
- `python scripts/metrics_manager.py effective`
- `python scripts/sql_runner.py --profile analytics --sql "SELECT region, SUM(amount) AS total FROM sales GROUP BY region" --output json`
