...
argument-hint: "add|list|test|import ..."
---

# /echart-dbconn

映射到 echart-skill 原始指令 `/dbconn`。

## 任务

管理 MySQL/PostgreSQL/MongoDB 连接，支持全局/项目级连接配置；PostgreSQL/MySQL 查询必须走 scripts/sql_runner.py。
...
- `/echart-dbconn list`
- `python scripts/sql_runner.py --profile analytics --sql "SELECT 1" --output json`
