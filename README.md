...
- 🎨 **专业 Dashboard 模板**：内置现代化 UI/UX 设计的 Dashboard 模板，支持卡片式布局、深色/浅色主题切换、响应式设计、自动刷新、导出 PDF、图表搜索等交互功能。
...
- 💻 **显性指令支持**：支持 `/import`、`/query`、`/chart`、`/dashboard` 等显性指令，让操作更精准。同时也支持自然语言的模糊匹配。
- 🔗 **外部数据库连接**：支持 MySQL、PostgreSQL、MongoDB、SQLite 等外部数据库查询和导入。
- 🔄 **数据轮询刷新**：支持定时从 HTTP API 或数据库自动刷新数据，适合实时监控场景。
- 🗺️ **三层级地图架构**：省份地图→ 城市地图（province.js）→ 区县地图，优先使用本地静态资源，精细维度自动降级到百度地图 API。
